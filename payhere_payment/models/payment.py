...
class PaymentAcquirerPayhere(models.Model):
    ...
    payhere_merchant_id = fields.Char(string="PayHere Merchant ID", required_if_provider='payhere',
...
class PaymentTransactionPayhere(models.Model):
    ...
    def _payhere_form_get_tx_from_data(self, data):
        ...
        if md5sign_check.upper() != md5sig.upper():
            raise ValidationError(('Payhere: invalid Md5sign, received %s, computed %s, for data %s')
                                    % (md5sig, md5sign_check, data))
